...
#### ✨ 核心功能与体验升级
- **新增会话管理双栏工作台**：把各个项目的本地会话集中在一处管理，左侧快速切会话，右侧直接看完整的执行轨迹时间线和调用上下文
- **会话标题智能生成与模型自选**：会话标题会自动跟着你的系统语言生成，也可以直接在页面里切换用 Gemini 官方模型来提炼；未登录账号时会贴心提示登录
- **工作区批量迁移与快速定位**：项目换了目录不用慌，现在支持一键批量迁移会话的工作区路径，也补齐了快速在访达/资源管理器中打开对应项目文件夹的操作
- **记住窗口状态与桌面原生通知**：关掉软件能记住窗口的尺寸和上次摆放位置，下次启动自动还原；接入了系统原生桌面通知，耗时任务完成时即时提醒；双击顶部标题栏也能顺畅缩放了
- **界面细节与长列表性能打磨**：全面理顺深浅色模式的配色细节，修掉了几处刺眼的色块；账号管理等长列表加上视口虚拟化，会话和账号再多滑起来也不卡了；顺带调大了用量大盘的核心数字

#### 🐛 稳定性与底层优化
- **超长会话加载与并发写入加固**：长会话改为首尾双向探测，避免内容被截断；写入元数据时加了互斥锁，多进程并发操作时不再互相踩踏
- **跨宿主孤立会话清理**：支持彻底清除掉残留的脏会话，同时修复了批量删除任务时偶尔吞掉取消信号的问题
- **工程瘦身与冷启动重构**：梳理了冷启动流水线并在退出时释放后台资源，统一底层网络连接池；新增 checkUnused 静态检测，顺手拔掉了多余的图片资源和死代码
...
#### ✨ Features & Improvements
- **New Session Management Workspace**: Browse and manage local sessions across projects in a clean dual-pane view; inspect full execution trajectories and context timelines with ease.
- **Smart Session Titles & Model Picker**: Session summaries now match your system language automatically; select official Gemini models directly from the session page to generate titles, with clear sign-in prompts.
- **Batch Workspace Migration**: Move sessions between directories in bulk with automatic path cleanup, plus one-click access to project folders in Finder or Explorer.
- **Window Memory & Native Notifications**: Remembers your window size and position on exit and restores it seamlessly on next launch; added native desktop notifications and title bar double-click zoom.
- **Smoother UI & Virtualized Lists**: Cleaned up light and dark theme colors, smoothed out dialogs and scrollbars, added virtualization to account lists for lag-free scrolling, and bumped up key usage metric font sizes.

#### 🐛 Stability & Infrastructure
- **Long Session Loading & Concurrency Locks**: Dual-ended probing prevents long conversation truncation, while write locks prevent race conditions under heavy concurrent sessions.
- **Orphan Session Cleanup**: Added tools to purge lingering cross-host sessions and fixed a bug where cancellation signals could get swallowed during batch deletion.
- **Leaner Build & Structured Lifecycle**: Extracted `AppBootstrapper` for clean startup/shutdown lifecycle, pooled shared HTTP connections, and integrated `checkUnused` to purge orphaned code and unused assets.
